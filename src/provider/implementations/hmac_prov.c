                     ...
                     size_t keylen, const OSSL_PARAM params[])
{
  ...
  macctx->operation = psa_mac_operation_init();

  if (key != NULL) {
    return hmac_setkey(macctx, key, keylen);
  }

  TEST_RESULT_TS(psa_mac_sign_setup(&macctx->operation, macctx->key, PSA_ALG_HMAC), PSA_SUCCESS, PROV_R_UNSUPPORTED_MAC_TYPE);
                       ...
                       size_t datalen)
{
  ...
  TEST_RESULT_TS(psa_mac_update(&macctx->operation, (const uint8_t *) data, datalen)), PSA_SUCCESS, PROV_R_UNSUPPORTED_MAC_TYPE);
  return OPENSSL_SUCCESS;
}
                      ...
                      size_t outsize)
{
  ...
  TEST_RESULT_TS(psa_mac_sign_finish(&macctx->operation, (uint8_t *) out, outsize, &hlen)), PSA_SUCCESS, PROV_R_UNSUPPORTED_MAC_TYPE);
  *outl = hlen;
  return OPENSSL_SUCCESS;
}
...
static int hmac_get_ctx_params(void *vmacctx, OSSL_PARAM params[])
{
  ...
  if ((p = OSSL_PARAM_locate(params, OSSL_MAC_PARAM_SIZE)) != NULL
      && !OSSL_PARAM_set_size_t(p, PSA_MAC_LENGTH(PSA_KEY_TYPE_HMAC, macctx->keylen, PSA_ALG_HMAC))) {
    return 0;
  }
