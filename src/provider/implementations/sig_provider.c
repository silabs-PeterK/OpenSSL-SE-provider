//TODO - get psa hash from openssl md, implement keymngm prov
...
static void *ecdsa_newctx(void *provctx, const char *propq)
{
  ...
  ctx = OPENSSL_zalloc(sizeof(PSA_ECDSA_CTX));
  if (ctx == NULL) {
    return NULL;
  }
...
{
  PSA_ECDSA_CTX *ctx = (PSA_ECDSA_CTX *)vctx;
  ctx->psa_hash = PSA_ALG_SHA_256;
  if (!ossl_prov_is_running()
      || ctx == NULL) {
    return 0;
  }
                      ...
                      size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
  ...
  if (ctx->nonce_type == 0) {
    alg = PSA_ALG_DETERMINISTIC_ECDSA(ctx->psa_hash);
  } else {
    alg = PSA_ALG_ECDSA(ctx->psa_hash);
  }

  size_t ecsize = PSA_SIGN_OUTPUT_SIZE(key_type, key_bits, alg);
  ...
  TEST_RESULT_TS(psa_sign_hash(ctx->key, alg, (const uint8_t *)tbs, tbslen,
                               (uint8_t *)sig, sigsize, siglen), PSA_SUCCESS, PROV_R_FAILED_TO_SIGN);

  TEST_EQUAL(siglen, ecsize, PROV_R_INVALID_SIGNATURE_SIZE);
  return OPENSSL_SUCCESS;
}
                        ...
                        const unsigned char *tbs, size_t tbslen)
{
  ...
  TEST_RESULT_TS(psa_verify_hash(ctx->key, PSA_ALG_ECDSA(ctx->psa_hash), (const uint8_t *)tbs, tbslen,
                                 (uint8_t *)sig, siglen), PSA_SUCCESS, PROV_R_INVALID_DIGEST);
