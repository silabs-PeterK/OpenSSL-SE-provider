                                     ...
                                     size_t *command_request_length)
{
  ...
  // Command type
  cpc_tx_data[current_command_size] = CMD_EXECUTE_SE_COMMAND;
  current_command_size += REQUEST_COMMAND_TYPE_SIZE;

  // SE command word
  memcpy(&cpc_tx_data[current_command_size],
         &cmd_ctx->command.command,
         REQUEST_COMMAND_SIZE);
  current_command_size += REQUEST_COMMAND_SIZE;

  // Num parameters
  uint8_t num_params = (uint8_t)cmd_ctx->command.num_parameters;
  cpc_tx_data[current_command_size] = num_params;
  current_command_size += REQUEST_NUM_PARAMS_SIZE;

  // Parameters
  VALIDATE_CPC_BUFFER_USAGE(current_command_size + (REQUEST_PARAMS_ELEMENT_SIZE * num_params));
  memcpy(&cpc_tx_data[current_command_size],
         cmd_ctx->command.parameters,
         REQUEST_PARAMS_ELEMENT_SIZE * num_params);
  current_command_size += (REQUEST_PARAMS_ELEMENT_SIZE * num_params);

  // Input number and lengths
  VALIDATE_CPC_BUFFER_USAGE(current_command_size + REQUEST_NUM_INPUTS_SIZE);
  size_t num_input_index = current_command_size;
  current_command_size += REQUEST_NUM_INPUTS_SIZE;
  ...
  if (cmd_ctx->command.data_in != NULL) {
    ...
    do {
      ...
      VALIDATE_CPC_BUFFER_USAGE(current_command_size + REQUEST_INPUT_LENGTHS_ELEMENT_SIZE);
      size_t input_data_len = current_data->length;
      memcpy(&cpc_tx_data[current_command_size],
             (uint32_t *)&input_data_len,
             REQUEST_INPUT_LENGTHS_ELEMENT_SIZE);
      current_command_size += REQUEST_INPUT_LENGTHS_ELEMENT_SIZE;
      current_data = current_data->next;
    } while (current_data != (void *)SE_DATATRANSFER_STOP);
  }

  // Output number and lengths
  VALIDATE_CPC_BUFFER_USAGE(current_command_size + REQUEST_NUM_OUTPUTS_SIZE);
  size_t num_output_index = current_command_size;
  current_command_size += REQUEST_NUM_OUTPUTS_SIZE;
  ...
  if (cmd_ctx->command.data_out != NULL) {
    ...
    do {
      ...
      VALIDATE_CPC_BUFFER_USAGE(current_command_size + REQUEST_OUTPUT_LENGTHS_ELEMENT_SIZE);
      size_t output_data_len = current_data->length;
      memcpy(&cpc_tx_data[current_command_size],
             (uint32_t *)&output_data_len,
             REQUEST_OUTPUT_LENGTHS_ELEMENT_SIZE);
      current_command_size += REQUEST_OUTPUT_LENGTHS_ELEMENT_SIZE;
      current_data = current_data->next;
    } while (current_data != (void *)SE_DATATRANSFER_STOP);
  }
...
static sl_status_t parse_response(sl_se_command_context_t *cmd_ctx)
{
  ...
  // Command type
  uint8_t type = cpc_rx_data[REQUEST_COMMAND_TYPE_OFFSET];
  if (type != CMD_EXECUTE_SE_COMMAND) {
    return SL_STATUS_COMMAND_IS_INVALID;
  }

  // SE command
  uint32_t command_word = *(uint32_t *)&cpc_rx_data[REQUEST_COMMAND_OFFSET];
  if (command_word != cmd_ctx->command.command) {
    return SL_STATUS_COMMAND_IS_INVALID;
  }

  // SE command response
  status = *(uint32_t *)&cpc_rx_data[RESPONSE_COMMAND_RESPONSE_OFFSET];
  if (status != SL_STATUS_OK) {
    return status;
  }

  uint8_t num_outputs = cpc_rx_data[RESPONSE_NUM_OUTPUTS_OFFSET];
  uint32_t *output_lengths = (uint32_t *)&cpc_rx_data[RESPONSE_OUTPUT_LENGTHS_OFFSET];

  SE_DataTransfer_t *current_data_output = cmd_ctx->command.data_out;
  size_t output_data_index = (RESPONSE_COMMAND_TYPE_SIZE
                              + RESPONSE_COMMAND_SIZE
                              + RESPONSE_COMMAND_RESPONSE_SIZE
                              + RESPONSE_NUM_OUTPUTS_SIZE
                              + (RESPONSE_OUTPUT_LENGTHS_ELEMENT_SIZE * num_outputs));
