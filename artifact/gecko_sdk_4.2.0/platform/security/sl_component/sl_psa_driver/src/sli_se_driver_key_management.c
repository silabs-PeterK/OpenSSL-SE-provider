...
static psa_status_t sli_se_driver_validate_key(
  const psa_key_attributes_t *attributes,
  const uint8_t *data,
  size_t data_length,
  size_t *bits)
      ...
      case PSA_ECC_FAMILY_SECP_R1: {
        if (PSA_KEY_TYPE_IS_ECC_KEY_PAIR(key_type)) { // Private key.
          ...
          *bits = psa_get_key_bits(attributes);
          ...
          #if (SLI_SE_KEY_PADDING_REQUIRED)
          ...
          *bits = (data_length - 1) * 8 / 2;
          ...
          switch (*bits) {
            ...
            case 224:
              #if defined(SLI_PSA_DRIVER_HSE_V1)
                // Series-2-config-1 devices do not support SECP224R1.
                return PSA_ERROR_NOT_SUPPORTED;
                break;
            ...
            #else // SLI_PSA_DRIVER_HSE_VAULT_HIGH
            ...
            #endif // SLI_PSA_DRIVER_HSE_VAULT_HIGH
...
{
  #if defined(SLI_PSA_DRIVER_FEATURE_ECC)
    psa_status_t status = sli_se_driver_validate_key(attributes,
                                                     data,
                                                     data_length,
                                                     bits);
    if (status != PSA_SUCCESS) {
